...
function firstNInQueueToArray(queue, numberOfValues) {
  var queueLength = Belt_MutableQueue.size(queue);
  var match = queueLength > numberOfValues;
  var maxValues = match ? numberOfValues : queueLength;
  return Belt_Array.makeBy(maxValues, (function (param) {
                return Belt_MutableQueue.popExn(queue);
              }));
}

function Make(Impl) {
  return (function (Promise$1) {
      ...
      var promiseCache = Hashtbl.create(undefined, 10);
      ...
      var dispatchQueue = function (_param) {
        while(true) {
          if (Belt_MutableQueue.isEmpty(queue) === false) {
      ...
      var addToQueue = function (item) {
        return Belt_MutableQueue.add(queue, item);
      };
      var load = function (key) {
        if (shouldCache && Hashtbl.mem(promiseCache, key)) {
          ...
          var promise = new Promise((function (resolve, reject) {
                  Belt_MutableQueue.add(queue, /* tuple */[
                        key,
                        resolve,
                        reject
                      ]);
                  if (Belt_MutableQueue.size(queue) === 1) {
                    if (shouldBatch) {
